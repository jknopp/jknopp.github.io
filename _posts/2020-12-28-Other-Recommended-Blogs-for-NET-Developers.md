---
title: Other Recommended Blogs for .NET Developers
date: 2020-12-28 01:00:00 -0700
# last_modified_at: 2016-03-09T16:20:02-05:00
description: Other blogs that I recommend folllowing for any developers in the .NET space.
categories: 
  - DotNet
tags:
  - dotnet
# header:
#    og_image: /assets/images/posts/header/my-cross-platform-development-environment.png
---

Here is a list of other blogs that I recommend following for any developers in the .NET space, in no particular order.

* [Markus Oberlehner](https://markus.oberlehner.net/)
* [Shawn Wildermuth](https://wildermuth.com/)
* [Steve Smith](https://ardalis.com/blog)
* [CSS Tricks](https://css-tricks.com/)
* [Jimmy Bogard](https://jimmybogard.com/)
* [Steve Gordon](https://www.stevejgordon.co.uk/)
* [Bryan Hogan](https://nodogmablog.bryanhogan.net/)
* [Kevin Dockx](https://www.kevindockx.com/?m=all)
